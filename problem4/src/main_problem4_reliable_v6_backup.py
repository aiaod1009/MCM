...
def chinese_summary(result):
    names = {"joint":"25点双环联合规划", "baseline":"原网格","compact":"45点方格","triangular":"31点三角网格","rings":"25点双环"}
...
def main():
    ...
    parser.add_argument("--search", choices=("joint", "rings", "triangular", "compact", "baseline"), default="joint")
    ...
    with log_path.open("w", encoding="utf-8") as log, (log_dir / ("actions_reliable_" + stamp + ".jsonl")).open("w", encoding="utf-8") as actions:
        def emit(message):
            ...
            log.write(message + "\n")
            log.flush()
        emit("可靠清除；搜索策略=" + {"joint":"25点双环联合规划", "rings":"25点双环", "triangular":"31点三角网格", "compact":"45点方格", "baseline":"原网格"}[args.search] + "；补测策略=" + ({"adaptive":"自适应侧向", "axial":"原直线"}[args.refinement] if not args.no_refine else "关闭"))
        client = PublicClient(args.url, args.robot_id, actions)
        solver_class = {"joint": JointSolver, "rings": RingSolver, "triangular": TriangularSolver, "compact": CompactSolver, "baseline": ReliableSolver}[args.search]
