...
class ReliableSolver:
    ...
    def validated_fallback_points(self, anchor, angle, region):
        return list(self.fallback_points(anchor, angle))
            ...
            for index in range(6):
                ...
                if kind == "direction" and geometry_valid:
                    ...
                    if clear_point is not None:
                        ...
                        break
        fallback = self.validated_fallback_points(anchor, angle, region if geometry_valid else None)
...
class CompactSolver(ReliableSolver):
    ...
    def select_batch_target(self, pending, anchor, remaining):
        return min(pending, key=lambda t: (distance(self.client.position, t[2]), t[0]))
    ...
    def solve(self):
        ...
        while remaining:
            ...
            while pending:
                target = self.select_batch_target(pending, point, remaining)
...
class JointSolver(RingSolver):
    """V6 rolling route order plus certified two-leg clear positions."""
    def __init__(self,*args,**kwargs):
        super().__init__(*args,**kwargs)
        self.continuation=None
        self.joint_saving_m=0.
        self.max_local_gap_m=0.
        self.max_batch_gap_m=0.
        self.batch_exact_calls=0
        self.batch_bounded_calls=0

    def probe_entry(self,anchor,result):
        if not self.refine:
            return next(iter(self.fallback_points(anchor,result["svd_deg"])))
        if self.refinement=="axial":
            a=math.radians(result["svd_deg"])
            return (anchor[0]+250*math.cos(a),anchor[1]+250*math.sin(a))
        region=add_bearing(initial_region(),anchor,result["svd_deg"])
        circle=covering_circle(region)
        if circle is None:
            return next(iter(self.fallback_points(anchor,result["svd_deg"])))
        ...
        a=math.radians(result["svd_deg"])
        u=(dx/norm,dy/norm) if norm>1e-8 else (math.cos(a),math.sin(a))
        v=(-u[1],u[0]);offset=min(250.,max(30.,radius/2.))
        return tuple(center[k]-offset*u[k]+offset*v[k] for k in (0,1))

    def select_batch_target(self,pending,anchor,remaining):
        terminal=self.next_scan_point(remaining) if remaining else None
        if len(self.cleared)+len(pending)==16:
            terminal=None
        entries=[self.probe_entry(anchor,t[1]) for t in pending]
        exits=[t[2] for t in pending]  # Observed-region estimates, not hidden source positions.
        order,gap,exact=batch_order(self.client.position,entries,exits,terminal)
        self.batch_exact_calls+=int(exact);self.batch_bounded_calls+=int(not exact)
        self.max_batch_gap_m=max(self.max_batch_gap_m,gap)
        self.continuation=entries[order[1]] if len(order)>1 else terminal
        self.emit("批次顺序：%d个目标；%s；估计模型差距%.3f米" %
                  (len(pending),"动态规划精确解" if exact else "有界改进",gap))
        return pending[order[0]]

    def clearance_point(self,region):
        point,gap,saving=two_leg_clear_point(region,self.client.position,self.continuation)
        if point is not None:
            self.joint_saving_m+=saving
            self.max_local_gap_m=max(self.max_local_gap_m,gap)
            self.emit("安全清除位置：两段路程减少%.3f米；数值优化差距%.3f米" % (saving,gap))
        return point

    def summary(self):
        result=super().summary()
        result.update(algorithm_version="reliable-v6",search="joint",
                      local_planned_saving_m=self.joint_saving_m,
                      max_local_numeric_gap_m=self.max_local_gap_m,
                      max_batch_estimate_gap_m=self.max_batch_gap_m,
                      exact_batch_plans=self.batch_exact_calls,bounded_batch_plans=self.batch_bounded_calls)
        return result
...
class FinalSolver(JointSolver):
    """V7 final: preserve V6 observations, prune provably futile fallback attempts."""
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.fallback_skipped = 0
        self.fallback_calls = 0

    def validated_fallback_points(self, anchor, angle, region):
        self.fallback_calls += 1
        points = feasible_strip_points(anchor, angle, region)
        self.fallback_skipped += 102-len(points)
        self.emit("可行域条带：保留%d/102点；剔除%d个不可能成功的尝试；原顺序不变。" %
                  (len(points),102-len(points)))
        return points

    def summary(self):
        result = super().summary()
        result.update(algorithm_version="reliable-v7", search="final",
                      fallback_calls=self.fallback_calls,
                      fallback_points_pruned=self.fallback_skipped)
        return result


def chinese_summary(result):
    names = {"final":"25点双环最终优化", "joint":"25点双环联合规划", "baseline":"原网格","compact":"45点方格","triangular":"31点三角网格","rings":"25点双环"}
    ...
    if "fallback_points_pruned" in result:
        lines.append("条带兜底目标数：%d" % result["fallback_calls"])
        lines.append("经几何裁剪剔除的候选点数：%d" % result["fallback_points_pruned"])
    if result.get("error"):
...
def main():
    ...
    parser.add_argument("--search", choices=("final", "joint", "rings", "triangular", "compact", "baseline"), default="final")
    ...
    with log_path.open("w", encoding="utf-8") as log, (log_dir / ("actions_reliable_" + stamp + ".jsonl")).open("w", encoding="utf-8") as actions:
        def emit(message):
            ...
            log.write(message + "\n")
            log.flush()
        emit("可靠清除；搜索策略=" + {"final":"25点双环最终优化", "joint":"25点双环联合规划", "rings":"25点双环", "triangular":"31点三角网格", "compact":"45点方格", "baseline":"原网格"}[args.search] + "；补测策略=" + ({"adaptive":"自适应侧向", "axial":"原直线"}[args.refinement] if not args.no_refine else "关闭"))
        client = PublicClient(args.url, args.robot_id, actions)
        solver_class = {"final": FinalSolver, "joint": JointSolver, "rings": RingSolver, "triangular": TriangularSolver, "compact": CompactSolver, "baseline": ReliableSolver}[args.search]
