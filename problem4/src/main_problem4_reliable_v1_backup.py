...
class ReliableSolver:
    def __init__(self, client, emit=print, refine=True):
    ...
    def clear_detected(self, channel, anchor, observation):
        ...
        if self.refine:
            a = math.radians(angle)
            for step in (250., 500., 750., 1000., 1250., 1500.):
    ...
    def summary(self):
        ...
        return dict(discovered_channels=sorted(self.discovered),
...
def main():
    ...
    with log_path.open("w", encoding="utf-8") as log, (log_dir / ("actions_reliable_" + stamp + ".jsonl")).open("w", encoding="utf-8") as actions:
        def emit(message):
            ...
            log.write(message + "\n")
            log.flush()
        emit("可靠清除基线；请仅在你准备好的演练测试中运行。")
        client = PublicClient(args.url, args.robot_id, actions)
        solver = ReliableSolver(client, emit, refine=not args.no_refine)
        entered = False
        error = None
        try:
            client.call("/enter")
