...
class CompactSolver(ReliableSolver):
    ...
    def solve(self):
        remaining = set(compact_scan_points())
        while remaining:
            point = (0., 0.) if self.scanned_points == 0 else min(
                remaining, key=lambda p: (distance(self.client.position, p), p))
            remaining.remove(point)
            pending = []
            self.emit("紧凑覆盖点%d/45：(%g, %g)" % (self.scanned_points + 1, *point))
            for channel in range(1, 21):
                if channel in self.cleared:
                    continue
                result = self.client.call("/measure", point, channel)
    ...
    def summary(self):
        ...
        result.update(algorithm_version="reliable-v3", search="compact",
                      planned_scan_points=len(compact_scan_points()),
                      scanned_points=self.scanned_points,
                      coverage_complete=self.scanned_points == len(compact_scan_points()),
...
def main():
    ...
    parser.add_argument("--search", choices=("compact", "baseline"), default="compact")
    ...
    with log_path.open("w", encoding="utf-8") as log, (log_dir / ("actions_reliable_" + stamp + ".jsonl")).open("w", encoding="utf-8") as actions:
        def emit(message):
            ...
            log.write(message + "\n")
            log.flush()
        emit("可靠清除；搜索策略=" + args.search + "；补测策略=" + (args.refinement if not args.no_refine else "none"))
        client = PublicClient(args.url, args.robot_id, actions)
        solver_class = CompactSolver if args.search == "compact" else ReliableSolver
